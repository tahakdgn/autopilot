...
- `windows/wrapper.cmd` - One `.cmd` template that `install.sh` copies under each terminal command's name on Windows, so bash scripts stay reachable from `cmd.exe` and Windows Terminal; it resolves its bash script through `%~n0`
